...
class SramedDataArray(implicit p: Parameters) extends AbstractBankedDataArray {
  ...
  (0 until LoadPipelineWidth).map(i => {
    ...
    val s0_read = io.read(i).fire
    val s1_read = RegNext(io.read(i).fire)
    val rr_read_fire = RegNext(RegNext(io.read(i).fire))
    val rr_div_addr = RegEnable(RegEnable(div_addrs(i), s0_read), s1_read)
    val rr_bank_addr = RegEnable(RegEnable(bank_addrs(i), s0_read), s1_read)
    val rr_way_addr = RegEnable(RegEnable(OHToUInt(way_en(i)), s0_read), s1_read)
