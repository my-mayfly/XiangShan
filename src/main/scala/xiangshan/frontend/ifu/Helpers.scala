...
trait IfuHelper extends HasXSParameter with HasIfuParameters {
  ...
  ///**
  //  * Preliminary adjustment of data transferred from ICache. Handles cases where instructions 
  //  * cross prediction blocks across cycles and within a single cycle.
  //  */
  //def recoverData(firstData: UInt, secondData: UInt, firstEndIdx: UInt, secondStartIdx: UInt, prevIsRvi: Bool): UInt = {
  //
  //}
