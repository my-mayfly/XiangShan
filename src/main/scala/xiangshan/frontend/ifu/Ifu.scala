...
class Ifu(implicit p: Parameters) extends IfuModule
  ...
  private val s1_fetchSize = VecInit.tabulate(FetchPorts) { i =>
    val start = s1_ftqFetch(i).startAddr(7, 0)
    val next  = s1_ftqFetch(i).nextStartAddr(7, 0)

    Mux(
      s1_ftqFetch(i).ftqOffset.valid,
      s1_ftqFetch(i).ftqOffset.bits + 1.U(log2Ceil(PredictWidth + 1).W),
      // (s1_ftqFetch(i).nextStartAddr - s1_ftqFetch(i).startAddr)(log2Ceil(PredictWidth + 1) + 1, 1)
      // -------- This point requires focused testing. ------- !!!
      (Cat(1.U(1.W), next) - Cat(0.U(1.W), start))(log2Ceil(PredictWidth + 1) + 1, 1)
    )
  }
  ...
  private val s1_ftrRange = VecInit.tabulate(FetchPorts)(i =>
    Fill(PredictWidth, s1_ftqFetch(i).ftqOffset.valid) | Fill(PredictWidth, 1.U(1.W)) >> ~getBasicBlockIdx(
      s1_ftqFetch(i).nextStartAddr,
      s1_ftqFetch(i).startAddr
    )
  )
  ...
  private val s2_totalInstrRange   = RegEnable((s1_instrRange(1) << s1_fetchSize(0)) | s1_instrRange(0), s1_fire)
  ...
  private val s2_iCacheAllRespWire =
    fromICache.valid &&
      fromICache.bits.vAddr(0) === s2_ftqFetch(0).startAddr &&
      (fromICache.bits.doubleline && fromICache.bits.vAddr(1) === s2_ftqFetch(0).nextlineStart || !s2_doubleline(0))
  ...
  preDecodeBounder.io.req.bits.cacheData :=
    Cat(s2_rawData, s2_rawData) >> Cat(s2_ftqFetch(0).startAddr(5, 0), 0.U(3.W))
  ...
  private val s2_fetchPcLowerResult = VecInit.tabulate(FetchPorts)(i =>
    VecInit((0 until PredictWidth).map(j =>
      Cat(0.U(1.W), s2_ftqFetch(i).startAddr(PcCutPoint - 1, 0)) + (j * 2).U
    ))
  ...
  for (i <- 0 until FetchPorts) {
    ...
    s2_fetchBlock(i).pcHigh          := s2_ftqFetch(i).startAddr(VAddrBits - 1, PcCutPoint)
    s2_fetchBlock(i).pcHighPlus1     := s2_ftqFetch(i).startAddr(VAddrBits - 1, PcCutPoint) + 1.U
    s2_fetchBlock(i).startAddr       := s2_ftqFetch(i).startAddr
    s2_fetchBlock(i).target          := s2_ftqFetch(i).nextStartAddr
  ...
  s3_alignBlockStartPos(s3_alignShiftNum) := s3_prevLastIsHalfRvi
  ...
  private val s3_alignInstrOffset   = alignData(s3_instrOffset, s3_alignShiftNum, s3_prevLastIsHalfRvi, false.B)
  ...
  private val s3_alignExceptionVec  = VecInit((0 until PredictWidth).map(i =>
    MuxCase(
      ExceptionType.None,
      Seq(
  ...
  for (i <- 0 until IBufferInPortNum) {
    ...
    val adjustedBlockHeadInst = Cat(s3_alignInstrData(i)(15, 0), s3_prevLastHalfData)
    s3_realAlignInstrData(i)  := Mux(s3_alignBlockStartPos(i), adjustedBlockHeadInst, s3_alignInstrData(i))
  }

  when(s3_fire && !s3_flush) {
    s3_prevLastHalfData := s3_alignInstrData(s3_instrCount - 1.U)(15, 0)
  }.elsewhen(s4_flush) {
  ...
  private val s4_alignPc  = VecInit.tabulate(IBufferInPortNum)(i =>
    catPC(
      ...
      Mux(s3_alignSelectBlock(i), s4_fetchBlock(1).pcHigh, s4_fetchBlock(0).pcHigh),
      Mux(s3_alignSelectBlock(i), s4_fetchBlock(1).pcHighPlus1, s4_fetchBlock(0).pcHighPlus1)
    )
  )
  ...
  io.toFtq.mmioCommitRead.valid      := RegNext(s4_reqIsMmio && s4_valid)
  ...
  private val s4_alignRealInstrValid  = Wire(Vec(PredictWidth, Bool()))
  ...
  io.toIBuffer.bits.valid         := s4_alignRealInstrValid.asUInt
  io.toIBuffer.bits.enqEnable     := checkerOutStage1.fixedTwoFetchRange.asUInt & s4_alignRealInstrValid.asUInt
  ...
  when(s4_reqIsMmio) {
    ...
    io.toIBuffer.bits.enqEnable   := s3_alignBlockStartPos.asUInt //s4_mmioRange.asUInt
  ...
  val firstRawPds   = WireDefault(VecInit.fill(IBufferInPortNum)(0.U.asTypeOf(new PreDecodeInfo)))
  val secondRawPds  = WireDefault(VecInit.fill(IBufferInPortNum)(0.U.asTypeOf(new PreDecodeInfo)))
