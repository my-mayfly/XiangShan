...
class RAS(implicit p: Parameters) extends BasePredictor {
  ...
  class RASStack(rasSize: Int, rasSpecSize: Int) extends XSModule with HasCircularQueuePtrHelper {
    ...
    when (io.spec_push_valid) {
      when(!io.spec_is_ret) {
        ...
        TOSR := TOSW
        TOSW := specPtrInc(TOSW)
        sctr := 0.U
        ssp  := Mux(sctr > 0.U, ptrInc(ssp), ssp)
      }
    }
    ...
    when (io.s3_cancel) {
      ...
      when (io.s3_missed_push) {
        ...
        when(!io.s3_is_ret) {
          ...
          TOSW := specPtrInc(io.s3_meta.TOSW)
          sctr := 0.U
          ssp  := Mux(io.s3_meta.sctr > 0.U, ptrInc(io.s3_meta.ssp), io.s3_meta.ssp)          
        }
      }
    }
    ...
    when (io.redirect_valid) {
      ...
      when (io.redirect_push_valid) {
        when (!io.redirect_is_ret){
          ...
          TOSW := specPtrInc(io.redirect_meta_TOSW)
          sctr := 0.U // Sacrifice part of the counter space to simplify the judgment
  ...
  s2_is_ret    := s2_full_pred.is_ret && s2_spec_push
  ...
  val s3_is_ret = io.in.bits.resp_in(0).s3.full_pred(2).is_ret && s3_push
  ...
  stack.redirect_pop_valid  := retMissPred
  stack.redirect_is_ret     := callMissPred && recover_cfi.pd.isRet
  ...
  stack.commit_is_ret     := updateValid && update.is_ret && update.is_call_taken
