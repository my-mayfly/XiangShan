...
trait Helpers extends HasMicroTageParameters with HalfAlignHelper {
  ...
  def computeHashIdx(pc: PrunedAddr, allFh: PhrAllFoldedHistories, tablesInfo: Seq[MicroTageInfo], tableId: Int)
  : UInt = {
  ...
  def computeHashTag(pc: PrunedAddr, allFh: PhrAllFoldedHistories, tablesInfo: Seq[MicroTageInfo], tableId: Int)
  : UInt = {
    val histLen = tablesInfo(tableId).HistoryLength
