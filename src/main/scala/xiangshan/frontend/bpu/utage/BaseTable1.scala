...
class MicroBaseTable1(
  ...
  private val oddEntries   = RegInit(VecInit(Seq.fill(numSets / 2)(0.U.asTypeOf(Vec(FetchBlockAlignInstNum, new MicroBaseEntry)))))
  private val evenEntries  = RegInit(VecInit(Seq.fill(numSets / 2)(0.U.asTypeOf(Vec(FetchBlockAlignInstNum, new MicroBaseEntry)))))
  ...
  private val alignTakenMask  = UIntToMask((io.req.startPc.toUInt)(FetchBlockAlignWidth - 1, instOffsetBits), FetchBlockAlignInstNum)
  ...
  private val readEntryVec = Mux(readNeedRotate,
    VecInit(readEvenEntryVec ++ readOddEntryVec),
    VecInit(readOddEntryVec ++ readEvenEntryVec)
  )
  ...
  io.resp.cfiPosition := Cat(readEntry.cfiPosition(CfiPositionWidth - 1) ^ readNeedRotate, readEntry.cfiPosition(CfiPositionWidth - 2, 0))

  // update
  private val trainIdx = (io.update.bits.startPc.toUInt)(log2Ceil(numSets / 2) + FetchBlockSizeWidth - 1, FetchBlockSizeWidth)
  ...
  private val oldCounters   = Mux(trainNeedRotate,
    VecInit(evenOldCounters ++ oddOldCounters),
    VecInit(oddOldCounters ++ evenOldCounters)
  )
  ...
  for(i <- 0 until FetchBlockAlignInstNum){
    when(((t1_needTrainValid(i) && !trainNeedRotate) || (t1_needTrainValid(FetchBlockAlignInstNum + i) && trainNeedRotate)) && io.update.valid) {
    ...
    when(((t1_needTrainValid(FetchBlockAlignInstNum + i) && !trainNeedRotate) || (t1_needTrainValid(i) && trainNeedRotate)) && io.update.valid) {
