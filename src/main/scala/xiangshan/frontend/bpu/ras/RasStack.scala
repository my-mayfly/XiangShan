...
class RasStack(implicit p: Parameters) extends RasModule
  ...
  private val nsp = RegInit(0.U(log2Up(StackSize).W))
  private val ssp = RegInit(0.U(log2Up(StackSize).W))
  ...
  when(writeBypassValidWire) {
    ...
    val popRedSsp  = Wire(UInt(log2Up(StackSize).W))
    ...
    val popSsp  = Wire(UInt(log2Up(StackSize).W))
  ...
  when(io.commit.popValid) {
    val nspUpdate = Wire(UInt(log2Up(StackSize).W))
