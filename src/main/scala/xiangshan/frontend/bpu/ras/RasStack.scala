...
class RasStack(implicit p: Parameters) extends RasModule
  ...
  class RasStackIO extends Bundle {
    ...
    class RasCommitIO extends Bundle {
      ...
      val metaSctr: UInt = Input(UInt(StackCounterWidth.W))
    }
...
//  )
  ...
  private val mismatchRasAction = (io.commit.popValid || io.commit.pushValid) &&
    (io.commit.metaSsp =/= nsp || io.commit.metaSctr =/= commitTop.ctr)
  XSPerfAccumulate("specNearOverflow", specNearOverflowed)
  XSPerfAccumulate("mismatchRasAction", mismatchRasAction)
  io.debug.bos := bos
  ...
  rasTrace.bits.commitTosw     := io.commit.metaTosw
  rasTrace.bits.commitSctr      := io.commit.metaSctr
  rasTrace.bits.tosw           := tosw
  rasTrace.bits.tosr           := tosr
  rasTrace.bits.bos            := bos
  rasTrace.bits.ssp            := ssp
  rasTrace.bits.nsp            := nsp
  rasTrace.bits.specNearOverflow   := specNearOverflowed

  private val rasTraceDBTables = ChiselDB.createTable(s"rasTrace", new RASTrace, EnableTrace)
