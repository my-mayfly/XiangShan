...
class TageBaseTable(implicit p: Parameters) extends TageModule with Helpers {
  ...
  sramBanks.zip(t0_setIdx).foreach {
    case (alignBank, setIdx) =>
      alignBank.zipWithIndex.foreach {
        case (bank, bankIdx) =>
          bank.io.r.req.valid       := t0_fire && t0_bankMask(bankIdx)
          bank.io.r.req.bits.setIdx := setIdx
      }
  }
  ...
  private val t2_alignBankIdx = RegEnable(t1_alignBankIdx, t1_fire)
  private val t2_bankMask     = RegEnable(t1_bankMask, t1_fire)
  private val t2_setIdx       = t1_setIdx.map(RegEnable(_, t1_fire))

  private val t2_oldCtrs = t1_oldCtrs.map(RegEnable(_, t1_fire))
  ...
  writeBuffers.zipWithIndex.foreach {
    case (alignBuffers, alignIdx) =>
      alignBuffers.zipWithIndex.foreach {
        case (buffer, bankIdx) =>
          buffer.io.write.head.valid          := t2_fire && t2_bankMask(bankIdx)
