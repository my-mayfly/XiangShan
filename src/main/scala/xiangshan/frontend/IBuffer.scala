...
class IBuffer(implicit p: Parameters) extends XSModule with HasCircularQueuePtrHelper with HasPerfEvents {
  ...
  bypassEntries.zipWithIndex.foreach {
    case (entry, idx) =>
      // Select
      val validOH = Range(0, needBypassNum).map {
        i =>
          io.in.bits.valid(i) &&
  ...
  // Data
  for (i <- 0 until IBufWriteBank) {
    bankedIBufWriteWire(i).zipWithIndex.foreach {
      case (entry, idx) => {
        // Select
        val validOH = Range(0, IBufEnqWidth / IBufWriteBank).map {
          j =>
            val normalMatch = enqPtrVec(enqBankOffset(i)(j)).value === (i + idx * IBufWriteBank).asUInt
            io.in.bits.valid(i + IBufWriteBank * j) && io.in.bits.enqEnable(i + IBufWriteBank * j) && normalMatch
        } // Should be OneHot
        val useBypassMatch = Range(0, DecodeWidth).map { k => enqPtrVec(k).value === (i + idx * IBufWriteBank).U}
        ...
        val writeEntry = Mux1H(validOH, enqBankEntrys(i))
        ibuf(i + idx * IBufWriteBank) := Mux(wen, writeEntry, ibuf(i + idx * IBufWriteBank))
        // Debug Assertion
        XSError(io.in.valid && PopCount(validOH) > 1.asUInt, "validOH is not OneHot")
      }
    }
  ...
  deqInBankPtrNext.zip(deqInBankPtr).zipWithIndex.foreach {
    case ((ptrNext, ptr), idx) => {
      ...
      val validIdx = Mux(
        ...
        idx.asUInt - deqBankPtr.value,
        ((idx + IBufReadBank).asUInt - deqBankPtr.value)(log2Ceil(IBufReadBank) - 1, 0)
