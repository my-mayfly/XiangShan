...
class RAS(implicit p: Parameters) extends BasePredictor {
  ...
  class RASStack(rasSize: Int, rasSpecSize: Int) extends XSModule with HasCircularQueuePtrHelper {
    ...
    def TOSRinRange(currentTOSR: RASPtr, currentTOSW: RASPtr):Bool = {
    ...
    def NOSinRange(currentTOSR: RASPtr, currentTOSW: RASPtr, currentNOS: RASPtr):Bool = {
      val inflightValid = WireInit(false.B)
      //This has some problem: !isBefore(currentNOS, BOS) && isBefore(currentNOS, currentTOSR) && isBefore(currentTOSR, currentTOSW)
