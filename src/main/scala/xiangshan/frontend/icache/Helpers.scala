...
trait ICacheMaybeRvcHelper extends HasICacheParameters with ICacheCacheLineHelper with HalfAlignHelper {
  ...
  def genInstRange(size: UInt): UInt = {
    ...
    val range = Mux(
      size(log2Ceil(MaxInstNumPerBlock)),
      Fill(MaxInstNumPerBlock, true.B),
      UIntToMask(size(log2Ceil(MaxInstNumPerBlock) - 1, 0), MaxInstNumPerBlock)
    )
    range
  }
  def genMaybeRvcShiftInfo(
    ...
    // shiftFlag is the shift-left direction used by shiftConfig for this line.
    info.shiftFlag   := !info.shouldShiftRight
    ...
    info.shiftConfig.zipWithIndex.foreach { case (c, i) =>
      ...
      info.sramShiftMaybeRvc(reqIdx)(portIdx) := shiftMaybeRvc(
    ...
    // consistent with the coordinate after shifting by sramShiftMaybeRvc / mshrShiftMaybeRvc.
    //
    ...
    info.maybeRvcMaskVec(0)(0) := genInstRange(MaxInstNumPerBlock.U - info.shiftNum(0)) & firstBlockRange
    ...
    info.maybeRvcMaskVec(1)(0) := genInstRange(req1Line0End) & totalBlockRange & ~firstBlockRange
    // (1)(1): req1's tail in line1 = the part of totalBlockRange not covered by firstBlockRange and (1)(0).
    info.maybeRvcMaskVec(1)(1) := totalBlockRange & ~(firstBlockRange | info.maybeRvcMaskVec(1)(0))
    info
  }
}
